...
static void ab32_pin_write(rt_device_t dev, rt_base_t pin, rt_base_t value)
{
    ...
    // if (GPIOF == (hal_sfr_t)(PORT_SFR(port))) {
    //     rt_kprintf("\n\n\n%ld   %d   %d\n\n\n", (hal_sfr_t)(PORT_SFR(port))[GPIOx], gpio_pin, (uint8_t)value);
    // }
    // if (GPIOF_BASE == (hal_sfr_t)(PORT_SFR(port))) {
    //     rt_kprintf("hhhhhhhhhhhhhhhhhhhhhh");
    // }
}
